...
def db_get_nace_dev_var(nace, bed_oms, variable_names):
    conn = psycopg2.connect(econm_url)
    cur = conn.cursor()

    query = sql.SQL("""
        SELECT orgnr, year, {vars}
    ...
    rows = cur.fetchall()
    columns = ["orgnr", "year"] + variable_names
...
def get_nace_dev_var(nace, variable_names, calculations, bed_oms=5000000):
    df = db_get_nace_dev_var(nace, bed_oms, variable_names)
    ...
    for year in range(2019, 2025):
        ...
        if not year_data.empty:
            return_dict[year] = make_stats(year_data)
        else:
            return_dict[year] = {}
